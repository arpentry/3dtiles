...
import { getTiffMetadata, readElevationData, generateTexture } from '../services/raster';
import { generateTerrainMesh, mapCoordinates, buildTriangleIndices } from '../services/mesh';
...
glb.get('/tileset.json', async (c) => {
  try {
    ...
    const root = createRootTile(square, TILESET_CENTER, minH, maxH, QUADTREE_MAX_LEVEL);
...
glb.get('/tiles/:level/:x/:y.glb', async (c) => {
  ...
  try {
    ...
    console.log(`   📍 Tile bounds: ${tileBounds.westDeg.toFixed(4)}°, ${tileBounds.southDeg.toFixed(4)}° to ${tileBounds.eastDeg.toFixed(4)}°, ${tileBounds.northDeg.toFixed(4)}°`);
