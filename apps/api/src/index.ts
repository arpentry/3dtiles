...
function validateTileCoordinates(
  ...
  if (isNaN(parsedLevel) || isNaN(parsedX) || isNaN(parsedY)) {
    return {
      valid: false,
      error: 'Invalid tile coordinates - must be numbers',
    };
  }
  ...
  if (parsedLevel < 0 || parsedLevel > QUADTREE_MAX_LEVEL) {
    return {
      valid: false,
      error: `Invalid level - must be between 0 and ${QUADTREE_MAX_LEVEL}`,
    };
  }
  ...
  if (parsedX < 0 || parsedY < 0) {
    return {
      valid: false,
      error: 'Invalid coordinates - must be non-negative',
    };
  }
...
const tileCache = async (c: Context, next: () => Promise<void>) => {
  const cacheDuration =
    c.env.TILE_CACHE_DURATION || TILE_CACHE_DURATION.toString();
  return cache({
    cacheName: 'tiles',
 ...
 */
app.get('/tiles/:level/:x/:y/tile.glb', tileCache, async (c: Context) => {
  ...
  try {
    ...
    const { data: elevationData, bbox: elevationBbox } =
      await readElevationDataFromGeoTiff(
    ...
    const texture = await readTextureDataFromGeoTiff(
