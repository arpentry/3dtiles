...
import { SphereHelper } from '3d-tiles-renderer/src/three/plugins/objects/SphereHelper.js';
import { EllipsoidRegionLineHelper } from '3d-tiles-renderer/src/three/plugins/objects/EllipsoidRegionHelper.js';
import {
  traverseAncestors,
  traverseSet,
} from '3d-tiles-renderer/src/core/renderer/tiles/traverseFunctions.js';
...
export class DebugTilesPlugin {
  ...
  _onTileVisibilityChange(tile, visible) {
    if (this.displayParentBounds) {
      traverseAncestors(tile, (current) => {
        ...
        const tileVisible =
          (current === tile && visible) ||
          (this.displayParentBounds && current[PARENT_BOUND_REF_COUNT] > 0);
  ...
  _createBoundHelper(tile) {
    function createThickBoxHelper(box, color) {
      console.log('box', box);

      // obb.box is a Box3 object with min/max properties
      ...
      const corners = [
        [-halfX, -halfY, -halfZ], // 0: min corner
        [halfX, -halfY, -halfZ], // 1
        [halfX, halfY, -halfZ], // 2
        [-halfX, halfY, -halfZ], // 3
        [-halfX, -halfY, halfZ], // 4
        [halfX, -halfY, halfZ], // 5
        [halfX, halfY, halfZ], // 6
        [-halfX, halfY, halfZ], // 7: max corner
      ];

      // Define only the 12 outer edges of the box
      const edges = [
        [0, 1],
        [1, 2],
        [2, 3],
      ...
      // Create a group to hold all the individual edge lines
      const group = new Group();

      // Create a separate Line2 for each edge
      edges.forEach(([i, j]) => {
        const positions = [...corners[i], ...corners[j]];
    ...
    if (obb) {
      ...
      const boxHelper = createThickBoxHelper(
        obb.box,
    ...
    if (sphere) {
      ...
      const sphereHelper = new SphereHelper(
        sphere,
        getIndexedRandomColor(tile.__depth),
      );
    ...
    if (region) {
      ...
      const regionHelper = new EllipsoidRegionLineHelper(
        region,
        getIndexedRandomColor(tile.__depth),
      );
  ...
  _updateHelperMaterial(tile, material) {
    // Handle both single materials and groups of materials
    const materials = Array.isArray(material) ? material : [material];

    materials.forEach((mat) => {
      if (mat && mat.material) {
        // Handle Line2 objects (mat.material is the actual material)
        const actualMaterial = mat.material;
        if (tile.__visible || !this.displayParentBounds) {
          actualMaterial.opacity = 1;
        } else {
          actualMaterial.opacity = 0.2;
        }

        const transparent = actualMaterial.transparent;
        actualMaterial.transparent = actualMaterial.opacity < 1;
        if (actualMaterial.transparent !== transparent) {
          actualMaterial.needsUpdate = true;
        }
  ...
  _updateBoundHelper(tile, visible) {
    ...
    if (
      ...
      cached.sphereHelper == null &&
      cached.regionHelper == null
    ) {
    ...
    if (!visible) {
      ...
      if (boxHelperGroup) {
        ...
        this._updateHelperMaterial(tile, boxHelperGroup.children);
      }
